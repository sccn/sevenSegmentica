% runicaSevenSegmentica() - Developed for an EEGLAB plugin pop_sevenSegmentica().
%                           It takes care of real-time update of the Seven Segmentica
%                           display. See pop_sevenSegmentica help.

% History:
% 10/08/2020 Makoto. Updated.
% 06/12/2019 Makoto. Modified.

% Copyright (C) 1996 Scott Makeig et al, SCCN/INC/UCSD, scott@sccn.ucsd.edu
%               2020 Makoto Miyakoshi,   SCCN/INC/UCSD, mmiyakoshi@ucsd.edu
%
...
function [weights,sphere,meanvar,bias,signs,lrates,data,y] = runicaSevenSegmentica(data,varargin) % NB: Now optionally returns activations as variable 'data' -sm 7/05

if nargin < 1
  help runicaDemoCustom  
  return
end
...
'runica(): pca value should be the number of principal components to retain')
   ...
   'runica() warning: kurtosis values inexact for << %d points.\n',...
      ...
      elseif strcmp(Keyword, lower('updateIntervalIdx')) % 06/12/2019 Makoto.
          switch Value
              case 1
                  updateInterval = NaN;
              case 2
      ...
      elseif strcmp(Keyword, lower('mixingMatrix')) % 10/02/2020 Makoto.
          mixingMatrix = Value;
...
if biasflag & extended
    while step < maxsteps, %%% ICA step = pass through all the data %%%%%%%%%
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%% Modified for sevenSegmentica (06/11/2019 Makoto) %%%
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if     isnan(updateInterval)
            % No action taken.
        elseif step == 1 | mod(step, updateInterval) == 0
            
            % Obtain icawinv.
            icawinv = pinv(weights*sphere);
            
            % Make icawinv positive and normalized to be 0<=x<=1
            for icIdx = 1:length(icawinv)
                if min(icawinv(:,icIdx))<0
                    icawinv(:,icIdx) = icawinv(:,icIdx)-min(icawinv(:,icIdx));
                end
            end
            icawinv = bsxfun(@rdivide, icawinv, max(icawinv,[],1));
            
            % Sort ICs by variance.     
            meanvar = sum(icawinv.^2).*sum((data').^2)/((chans*frames)-1); % from Rey Ramirez 8/07
            [sortvar, windex] = sort(meanvar);
            windex = windex(ncomps:-1:1); % order large to small
            icawinv = icawinv(:,windex);
            
            % Plot the final results.           
            draw7SegIndependently(findobj('Tag','ic1'), icawinv(:,1));
            draw7SegIndependently(findobj('Tag','ic2'), icawinv(:,2));
            draw7SegIndependently(findobj('Tag','ic3'), icawinv(:,3));
            draw7SegIndependently(findobj('Tag','ic4'), icawinv(:,4));
            draw7SegIndependently(findobj('Tag','ic5'), icawinv(:,5));
            draw7SegIndependently(findobj('Tag','ic6'), icawinv(:,6));
            draw7SegIndependently(findobj('Tag','ic7'), icawinv(:,7));
...
if biasflag & ~extended
    while step < maxsteps, %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        %%% Modified for sevenSegmentica (10/02/2020 Makoto) %%%
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        if     isnan(updateInterval)
            % No action taken.
        elseif step == 1 | mod(step, updateInterval) == 0
            ...
            if minRV < 0.1
                set(findobj('Tag','rv1Text'), 'String', sprintf('min r.v. %.1f%%', minRV*100), 'ForegroundColor', [1 0 0]);
            else
                set(findobj('Tag','rv1Text'), 'String', sprintf('min r.v. %.1f%%', minRV*100), 'ForegroundColor', [0 0 0.4]);
            end
            
            minRV = min(sum(bsxfun(@minus, mixingMatrix, icawinvPlot(:,2)).^2)/sum(icawinvPlot(:,2).^2));
            if minRV > 1, minRV = 1; end
            if minRV < 0.1
                set(findobj('Tag','rv2Text'), 'String', sprintf('min r.v. %.1f%%', minRV*100), 'ForegroundColor', [1 0 0]);
            else
                set(findobj('Tag','rv2Text'), 'String', sprintf('min r.v. %.1f%%', minRV*100), 'ForegroundColor', [0 0 0.4]);
            end
            
            minRV = min(sum(bsxfun(@minus, mixingMatrix, icawinvPlot(:,3)).^2)/sum(icawinvPlot(:,3).^2));
            if minRV > 1, minRV = 1; end
            if minRV < 0.1
                set(findobj('Tag','rv3Text'), 'String', sprintf('min r.v. %.1f%%', minRV*100), 'ForegroundColor', [1 0 0]);
            else
                set(findobj('Tag','rv3Text'), 'String', sprintf('min r.v. %.1f%%', minRV*100), 'ForegroundColor', [0 0 0.4]);
            end
            
            minRV = min(sum(bsxfun(@minus, mixingMatrix, icawinvPlot(:,4)).^2)/sum(icawinvPlot(:,4).^2));
            if minRV > 1, minRV = 1; end
            if minRV < 0.1
                set(findobj('Tag','rv4Text'), 'String', sprintf('min r.v. %.1f%%', minRV*100), 'ForegroundColor', [1 0 0]);
            else
                set(findobj('Tag','rv4Text'), 'String', sprintf('min r.v. %.1f%%', minRV*100), 'ForegroundColor', [0 0 0.4]);
            end
            
            minRV = min(sum(bsxfun(@minus, mixingMatrix, icawinvPlot(:,5)).^2)/sum(icawinvPlot(:,5).^2));
            if minRV > 1, minRV = 1; end
            if minRV < 0.1
                set(findobj('Tag','rv5Text'), 'String', sprintf('min r.v. %.1f%%', minRV*100), 'ForegroundColor', [1 0 0]);
            else
                set(findobj('Tag','rv5Text'), 'String', sprintf('min r.v. %.1f%%', minRV*100), 'ForegroundColor', [0 0 0.4]);
            end
            
            minRV = min(sum(bsxfun(@minus, mixingMatrix, icawinvPlot(:,6)).^2)/sum(icawinvPlot(:,6).^2));
            if minRV > 1, minRV = 1; end
            if minRV < 0.1
                set(findobj('Tag','rv6Text'), 'String', sprintf('min r.v. %.1f%%', minRV*100), 'ForegroundColor', [1 0 0]);
            else
                set(findobj('Tag','rv6Text'), 'String', sprintf('min r.v. %.1f%%', minRV*100), 'ForegroundColor', [0 0 0.4]);
            end
            
            minRV = min(sum(bsxfun(@minus, mixingMatrix, icawinvPlot(:,7)).^2)/sum(icawinvPlot(:,7).^2));
            if minRV > 1, minRV = 1; end
            if minRV < 0.1
                set(findobj('Tag','rv7Text'), 'String', sprintf('min r.v. %.1f%%', minRV*100), 'ForegroundColor', [1 0 0]);
            else
                set(findobj('Tag','rv7Text'), 'String', sprintf('min r.v. %.1f%%', minRV*100), 'ForegroundColor', [0 0 0.4]);
            end
